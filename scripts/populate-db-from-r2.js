...
try {
    ...
    try {
        ...
        console.error('❌ Could not load @prisma/client. Run pnpm db:generate');
        process.exit(1);
    }
}
...
try {
    ...
    try {
        ({ S3Client, ListObjectsV2Command } = require('../apps/api/node_modules/@aws-sdk/client-s3'));
    } catch (e2) {
         console.error('❌ Could not load @aws-sdk/client-s3.');
         process.exit(1);
    }
}
...
async function main() {
    ...
    if (!systemUser) {
        ...
        const { v4: uuidv4 } = require('uuid'); // fallback if needed
    ...
    for (const file of videoFiles) {
        ...
        try {
            ...
            console.error(`\n❌ Failed to sync file: ${file.Key}`, e.message);
        }
    }
