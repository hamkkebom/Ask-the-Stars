...
const R2_PUBLIC_URL =
  process.env.CLOUDFLARE_PUBLIC_Url || process.env.NEXT_PUBLIC_R2_PUBLIC_URL;
...
async function migrate() {
  console.log(
    `🚀 Starting R2 -> Stream Migration ` + (DRY_RUN ? '(DRY RUN)' : '')
  );
  ...
  try {
    ...
    const videosToMigrate = await prisma.videoTechnicalSpec.findMany({
      where: {
        AND: [{ r2Key: { not: null } }, { streamUid: null }],
      },
    ...
    console.log(
      `found ${videosToMigrate.length} videos eligible for migration.`
    );
