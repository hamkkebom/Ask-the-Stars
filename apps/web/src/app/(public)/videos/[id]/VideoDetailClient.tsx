              ...
              <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/5">
                <div className="flex flex-wrap gap-2 mb-4">
                  {video.categories.map((cat: string) => (
                    <span
                      key={cat}
                      className="px-3 py-1 bg-vibrant-cyan/20 text-vibrant-cyan text-xs font-bold rounded-full border border-vibrant-cyan/20"
                    >
                      #{cat}
                    </span>
                  ))}
                  {video.tags.map((tag: string) => (
                    <span
                      key={tag}
                      className="px-3 py-1 bg-white/5 text-neutral-400 text-xs font-medium rounded-full border border-white/5"
                    >
                      #{tag}
                    </span>
                  ))}
                ...
                <p className="text-neutral-300 leading-relaxed whitespace-pre-line">
                  {video.description ||
                    '이 영상에 대한 설명이 아직 없습니다. 영상을 감상해보세요! 💜'}
                </p>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {recommendations.map((video) => (
                      <CompactVideoCard
                        key={video.id}
                        {...video}
                        isNew={video.matchScore ? video.matchScore > 85 : false}
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="flex gap-4 mb-8">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-end mt-2">
                    <button className="px-4 py-2 bg-neutral-800 rounded-lg text-sm text-neutral-400 hover:text-white hover:bg-neutral-700 transition-colors">
                      등록
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-16 h-16 rounded-full border-2 border-vibrant-cyan/30 p-0.5">
                    <div className="w-full h-full rounded-full overflow-hidden relative bg-gradient-to-br from-vibrant-cyan/20 to-vibrant-magenta/20 flex items-center justify-center">
                      {video.counselor?.avatarUrl ? (
                        <Image
                          src={video.counselor.avatarUrl}
                          alt={video.counselor.name || '상담사'}
                          fill
                          className="object-cover"
                        />
                      ) : (
                        <span className="text-2xl font-bold text-white/60">
                          {(video.counselor?.name || '상')[0]}
                        </span>
                      )}
                    </div>
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-white flex items-center gap-2">
                      ...
                      <span className="px-1.5 py-0.5 bg-black/40 border border-vibrant-cyan/50 rounded text-[10px] text-vibrant-cyan">
                        Official
                      </span>
                    </h3>
                    <p className="text-sm text-neutral-400">
                      {video.counselor.role}
                    </p>
                  </div>
                </div>
                <div className="mt-4">
                ...
                {creatorVideos.map((v) => (
                  <div key={v.id} className="group cursor-pointer">
                    <div className="relative aspect-video rounded-lg overflow-hidden mb-2 border border-white/5 bg-neutral-800">
                      {v.thumbnailUrl &&
                      v.thumbnailUrl !== '/placeholder.jpg' ? (
                        <Image
                          src={v.thumbnailUrl}
                          alt={v.title}
                          fill
