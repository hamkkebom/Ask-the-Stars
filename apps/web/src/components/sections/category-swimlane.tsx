...
export function CategorySwimlane({
  title,
  category,
  viewAllLink,
  ...
  const videoItems: VideoProps[] = videos.map((v: any) => {
    ...
    const creatorName =
      v.freelancer?.name || (v.maker?.name && v.maker.name !== 'System Admin')
    ...
    const counselorName =
      v.project?.counselor?.name && v.project.counselor.name !== 'System Admin'
        ...
        >
          {isLoading
            ? // Skeleton Loader
              Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="min-w-[280px] md:min-w-[320px] aspect-video bg-white/5 animate-pulse rounded-lg"
                />
              ))
          ...
          {viewAllLink && (
            <div className="min-w-[150px] flex items-center justify-center group/viewall">
              <Link
                href={viewAllLink}
                className="flex flex-col items-center gap-3 p-4 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 hover:border-vibrant-cyan transition-all group-hover/viewall:scale-110"
              >
