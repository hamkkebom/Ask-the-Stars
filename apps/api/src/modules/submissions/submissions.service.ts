...
export class SubmissionsService {
  ...
  async create(userId: string, dto: CreateSubmissionDto): Promise<any> {
    ...
    if (!assignmentId) {
      ...
      if (!projectId)
        throw new NotFoundException('Assignment ID or Project ID is required.');
    }
  ...
  async findOne(id: string): Promise<any> {
    ...
    if (submission.streamUid) {
        signedToken = await this.cloudflare.generateSignedToken(
        submission.streamUid
      );
    }
  ...
  async update(
    ...
    const isAdmin = ['ADMIN', 'MOON_MANAGER', 'MOON_FEEDBACK'].includes(
      user.role
    );
  ...
  async generateUploadUrl(
    ...
    const uploadUrl = await this.cloudflare.getDirectUploadUrl(
      userId,
      uploadLength,
      metadata
    );
  ...
  async uploadCaption(id: string, language: string, fileBuffer: Buffer) {
    ...
    return this.cloudflare.uploadCaption(
      submission.streamUid,
      language,
      fileBuffer
    );
