...
export default function ClientSignupPage() {
  ...
  return (
    <div>
      <Link href="/auth/signup" className="inline-flex items-center gap-1 text-[#aaa] hover:text-white transition-colors mb-6">
        <ArrowLeft className="w-4 h-4" />
        뒤로
      </Link>

      <h1 className="text-2xl font-bold text-white text-center mb-2">클라이언트 가입</h1>
      <p className="text-[#aaa] text-center mb-8">영상 제작을 의뢰하세요</p>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-[#aaa] mb-2">이름 *</label>
        ...
        <div>
          <label className="block text-sm font-medium text-[#aaa] mb-2">회사명</label>
          <input name="company" type="text" value={formData.company} onChange={handleChange} placeholder="(선택)"
            className="w-full bg-[#121212] border border-[#3f3f3f] rounded-lg px-4 py-3 text-white placeholder-[#666] focus:outline-none focus:border-blue-500" />
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-[#aaa] mb-2">비밀번호 *</label>
          <div className="relative">
            <input name="password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={handleChange} placeholder="8자 이상"
        ...
        <div>
          ...
          <input name="confirmPassword" type="password" value={formData.confirmPassword} onChange={handleChange} placeholder="비밀번호 재입력"
            className="w-full bg-[#121212] border border-[#3f3f3f] rounded-lg px-4 py-3 text-white placeholder-[#666] focus:outline-none focus:border-blue-500" />
        </div>

        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" name="agreeTerms" checked={formData.agreeTerms} onChange={handleChange} className="mt-1 w-4 h-4" />
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? <><Loader2 className="w-5 h-5 animate-spin" />가입 중...</> : '가입하기'}
        </button>
      </form>
    </div>
  );
