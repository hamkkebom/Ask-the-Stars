  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#212121] rounded-xl border border-[#3f3f3f] overflow-hidden">
          ...
          <div className="divide-y divide-[#3f3f3f]">
            <Link href="/admin/videos?status=REVIEWING" className="flex items-center justify-between px-5 py-3 hover:bg-[#3f3f3f] transition-colors">
            ...
            <Link href="/admin/settlements?status=PENDING" className="flex items-center justify-between px-5 py-3 hover:bg-[#3f3f3f] transition-colors">
            ...
            <Link href="/admin/clients" className="flex items-center justify-between px-5 py-3 hover:bg-[#3f3f3f] transition-colors">
            ...
            {activities.length === 0 ? (
              ...
              <div className="divide-y divide-[#3f3f3f]">
                {activities.map((activity) => (
                  <div key={`${activity.type}-${activity.id}`} className="px-5 py-3 flex items-start gap-3">
                    <div className={`mt-1 w-2 h-2 rounded-full ${
                      activity.type === 'video_upload' ? 'bg-green-400' : 'bg-blue-400'
                    }`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white truncate">
                        {activity.type === 'video_upload' ? '영상 업로드: ' : '프로젝트 생성: '}
                        {activity.title}
                      </p>
                      <p className="text-xs text-[#666]">
                        {activity.actor} · {new Date(activity.timestamp).toLocaleString('ko-KR')}
                      </p>
                    </div>
                  </div>
                ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Link href="/admin/videos" className="bg-[#212121] rounded-xl border border-[#3f3f3f] p-4 hover:bg-[#3f3f3f] transition-colors text-center">
          <Video className="w-6 h-6 text-purple-400 mx-auto mb-2" />
          <span className="text-white text-sm">영상 관리</span>
        </Link>
        <Link href="/admin/freelancers" className="bg-[#212121] rounded-xl border border-[#3f3f3f] p-4 hover:bg-[#3f3f3f] transition-colors text-center">
          <Users className="w-6 h-6 text-blue-400 mx-auto mb-2" />
          <span className="text-white text-sm">프리랜서 관리</span>
        </Link>
        <Link href="/admin/projects" className="bg-[#212121] rounded-xl border border-[#3f3f3f] p-4 hover:bg-[#3f3f3f] transition-colors text-center">
          <FolderKanban className="w-6 h-6 text-green-400 mx-auto mb-2" />
          <span className="text-white text-sm">프로젝트 관리</span>
        </Link>
        <Link href="/admin/settlements" className="bg-[#212121] rounded-xl border border-[#3f3f3f] p-4 hover:bg-[#3f3f3f] transition-colors text-center">
