...
export default function ReviewDetailPage({
  ...
  const [selectedVersionA, setSelectedVersionA] = useState<Submission | null>(
    null
  );
  const [selectedVersionB, setSelectedVersionB] = useState<Submission | null>(
    null
  );
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-white">
            {submission.assignment?.request.title || submission.project?.title}
          </h1>
          <p className="text-gray-400 text-sm">
            {submission.user?.name} · v{submission.version}
          </p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            >
              {viewMode === 'single' ? (
                ...
                <div className="flex flex-row gap-2 h-full">
                  ...
                  <div className="flex-1 h-full">
                    <VideoPlayerComp
                      submission={selectedVersionB}
                      label={`v${selectedVersionB?.version}`}
                      isPlaying={isPlayingB}
                      onToggle={() => setIsPlayingB(!isPlayingB)}
                      aspectRatio={aspectRatio}
                      isMuted={isMuted}
                      setDuration={() => {}}
                      setCurrentTime={() => {}}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 py-3 bg-green-600 rounded-lg font-bold text-white"
            >
              승인
            </button>
            <button
              onClick={() => setShowActionModal('revision')}
              className="flex-1 py-3 bg-orange-600 rounded-lg font-bold text-white"
            >
              수정요청
            </button>
            <button
              onClick={() => setShowActionModal('reject')}
              className="flex-1 py-3 bg-red-600 rounded-lg font-bold text-white"
            >
              반려
            </button>
          </div>
          ...
          <GlassCard className="p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-neutral-500">제출일</span>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">v{submission.version}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-neutral-500">노트</span>
                <span className="text-white">{submission.notes || '-'}</span>
              </div>
            </div>
          </GlassCard>
            ...
            <GlassCard className="p-4">
              ...
              <div className="space-y-2">
                {versions.map((v) => (
                  <button
                    key={v.id}
                    onClick={() => setSelectedVersionA(v)}
                    className={cn(
                      'w-full text-left p-2 rounded text-sm transition-colors',
                      selectedVersionA?.id === v.id
                        ? 'bg-primary text-white'
          ...
          <GlassCard className="w-full max-w-md p-6">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 bg-white/5 rounded text-neutral-400"
              >
                취소
              </button>
              <button
                onClick={handleActionConfirm}
                className="flex-1 py-2 bg-primary rounded font-bold text-white"
              >
                확인
              </button>
            </div>
